...
func (s *Service) UpdateUser(ctx context.Context, userID string, input UpdateUserInput) (*User, error) {
	...
	if input.FirstName != nil {
		...
		if len(value) < 2 || len(value) > 50 {
			return nil, fmt.Errorf("%w: firstName must be 2 to 50 characters", ErrInvalidInput)
		}
	}
	if input.LastName != nil {
		value := strings.TrimSpace(*input.LastName)
		if len(value) < 2 || len(value) > 50 {
			return nil, fmt.Errorf("%w: lastName must be 2 to 50 characters", ErrInvalidInput)
		}
	}
	if input.Email != nil {
		if _, err := mail.ParseAddress(*input.Email); err != nil {
			return nil, fmt.Errorf("%w: email format is invalid", ErrInvalidInput)
		}
	}
	if input.Phone != nil {
		if !phoneRegex.MatchString(*input.Phone) {
			return nil, fmt.Errorf("%w: phone format is invalid", ErrInvalidInput)
		}
	}
	if input.Age != nil && *input.Age <= 0 {
		return nil, fmt.Errorf("%w: age must be positive", ErrInvalidInput)
	}
	if input.Status != nil && *input.Status != StatusActive && *input.Status != StatusInactive {
		return nil, fmt.Errorf("%w: status must be Active or Inactive", ErrInvalidInput)
	}
